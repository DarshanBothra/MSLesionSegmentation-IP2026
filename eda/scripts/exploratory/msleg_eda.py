...
DATASET_ROOT = "/home/darshan/MS/data/RAW/MSLegSeg"
...
OUTPUT_JSON_PATH = "/home/darshan/MS/eda/results/exploratory/msleg_eda.json"
        ...
        for patient in patients:
            ...
            for tp in time_points:
                if mode in output["patient_wise"][patient][tp]["present"]:
                    img_dims, orientation = get_img_dims(TRAIN_ROOT, patient, tp, mode) if mode != "MASK" else get_img_dims(MASK_ROOT, patient, tp, mode)
    ...
    for patient in patients:
        ...
        for tp in time_points:
            if output["patient_wise"][patient][tp]["mask_present"]:
                img_dims, orientation = get_img_dims(MASK_ROOT, patient, tp, "MASK")
                img_dims, orientation = str(img_dims), str(orientation)
