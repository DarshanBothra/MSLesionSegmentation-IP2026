...
DATASET_ROOT = "/home/darshan/MS/data/ISBI2015"
TRAIN_ROOT = os.path.join(DATASET_ROOT, "train")
TEST_ROOT = os.path.join(DATASET_ROOT, "test") # cannot be used, no mask present!
OUTPUT_JSON_PATH = "/home/darshan/MS/eda/results/isbi_eda.json"
        ...
        for patient in patients:
            for tp in time_points:
                if mode in patient_record[tp]["present"]:
                    img_dims = get_img_dims(TRAIN_ROOT, patient, tp, mode)
    ...
    for patient in patients:
        for tp in time_points:
            if output["patient_wise"][patient][tp]["mask_present"]:
                img_dims = get_img_dims(TRAIN_ROOT, patient, tp, "mask")
                img_dims = str(img_dims)
