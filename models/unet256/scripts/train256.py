...
train256.py
-----------
End-to-end training script for the 2-D U-Net on the MS preprocessed 256x256 multimodal dataset.
...
from model256 import Model2D
from dataset256 import build_datasets, extract_slices
...
def build_model(lr: float = 1e-5, optimizer_name: str = "adam") -> tf.keras.Model:
    """Instantiate Model2D and compile it with Dice loss, DiceScore, IoUScore."""
...
def plot_training_curves(history, run_dir: str):
    ...
    _save("dice_score", "val_dice_score",
          "Dice Coefficient",
          "2-D U-Net (256x256) — Dice Coefficient vs Epochs (T1w + T2w + FLAIR)",
          "dice_curve.png")

    _save("iou_score", "val_iou_score",
          "IoU Score",
          "2-D U-Net (256x256) — IoU Score vs Epochs (T1w + T2w + FLAIR)",
          "iou_curve.png")
...
def main():
    parser = argparse.ArgumentParser(description="Train 2-D U-Net (256x256) for multi-modal brain lesion segmentation")
