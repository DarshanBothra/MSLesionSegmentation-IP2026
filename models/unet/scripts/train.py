...
train.py
--------
End-to-end training script for the 2-D U-Net on the MS preprocessed multimodal dataset.
...
def build_model(lr: float = 1e-5, optimizer_name: str = "adam") -> tf.keras.Model:
    ...
    m2d = Model2D(IMG_HEIGHT=182, IMG_WIDTH=218, IMG_CHANNELS=3)
    m2d.initializeModel()
...
def plot_training_curves(history, run_dir: str):
    ...
    _save("dice_score", "val_dice_score",
          "Dice Coefficient",
          "2-D U-Net — Dice Coefficient vs Epochs (T1w + T2w + FLAIR)",
          "dice_curve.png")

    _save("iou_score", "val_iou_score",
          "IoU Score",
          "2-D U-Net — IoU Score vs Epochs (T1w + T2w + FLAIR)",
          "iou_curve.png")
...
def main():
    parser = argparse.ArgumentParser(description="Train 2-D U-Net for multi-modal brain lesion segmentation")
