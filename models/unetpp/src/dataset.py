...
def centre_pad_crop_2d(arr: np.ndarray, target_h: int = 256, target_w: int = 256) -> np.ndarray:
...
def extract_slices(t1_path: str,
    ...
    for z in range(depth):
        ...
        # Symmetrically pad/crop each modality slice to match the exact target size if needed
...
def make_generator_2d(subjects_list, preprocessed_dir, target_h=256, target_w=256,
                      skip_blank_ratio=0.95, is_train=True, shuffle=False):
    """Creates a generator that yields 2D slices patient-by-patient to avoid Host RAM depletion."""
    def generator():
...
def build_datasets(preprocessed_dir: str,
    ...
    and return generator-backed tf.data.Datasets to avoid Host RAM depletion.
    """
    ...
    output_signature = (
        tf.TensorSpec(shape=(target_h, target_w, 3), dtype=tf.float32),
        tf.TensorSpec(shape=(target_h, target_w, 1), dtype=tf.float32)
    )
    
    train_gen = make_generator_2d(train_subjs, preprocessed_dir, target_h, target_w,
                                  skip_blank_ratio=skip_blank_ratio, is_train=True, shuffle=True)
    val_gen = make_generator_2d(val_subjs, preprocessed_dir, target_h, target_w,
                                skip_blank_ratio=0.0, is_train=False, shuffle=False)
    test_gen = make_generator_2d(test_subjs, preprocessed_dir, target_h, target_w,
                                 skip_blank_ratio=0.0, is_train=False, shuffle=False)
    ...
    # Shuffle and batch dynamically in memory
    # buffer_size=8000 uses ~8.0 GB of RAM (approx 10% of system memory) for near-global shuffling
    train_ds = train_ds.shuffle(buffer_size=8000).batch(batch_size).prefetch(tf.data.AUTOTUNE)
    val_ds = val_ds.batch(batch_size).prefetch(tf.data.AUTOTUNE)
    test_ds = test_ds.batch(batch_size).prefetch(tf.data.AUTOTUNE)
    
    return train_ds, val_ds, test_ds, {
        "train": (None, None, train_subjs),
        "val": (None, None, val_subjs),
        "test": (None, None, test_subjs)
    }
