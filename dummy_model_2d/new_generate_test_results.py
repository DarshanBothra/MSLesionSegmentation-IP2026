...
def main():
    ...
    model_path = '/home/darshan/MS/dummy_model_2d/runs/model3/best_model.h5'
    output_dir = '/home/darshan/MS/dummy_model_2d/test_predictions/test_model'
    nii_output_dir = '/home/darshan/MS/dummy_model_2d/test_predictions/test_model/nii'
    ...
    for i, patient in enumerate(selected_patients):
        ...
        affine = nib.load(os.path.join(patient_dir, 'flair.nii.gz')).affine
        header = nib.load(os.path.join(patient_dir, 'flair.nii.gz')).header
        ...
        # Predict and store best middle slice
        ...
        slices = np.empty((H, W, 0))
        for best_z in range(D):
            ...
            # Unpad
            pred_unpad = unpad_2d(pred_bin, H, W)
            # print(pred_unpad.shape)

            slices = np.concatenate((slices, pred_unpad[:, :, np.newaxis]), axis=2)

        # print(slices.shape)

        # Package NII file
        pred_nii = nib.Nifti1Image(slices, affine, header)
        nib.save(pred_nii, os.path.join(nii_output_dir, f"{patient}_prediction.nii.gz"))
